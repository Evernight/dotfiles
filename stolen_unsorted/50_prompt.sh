# Modified version of file from https://github.com/cowboy/dotfiles
...
function prompt_git() {
  ...
  if [[ "$flags" ]]; then
    output="$c5$output $flags"
  fi
  echo "$sep $c0$output $c9"
}
...
function prompt_command() {
  ...
  # date: [HH:MM:SS]
  #PS1="$PS1$c1[$c4$(date +"%H$c1:$c4%M$c1:$c4%S")$c1]$c9"
  PS1="$PS1$c4$(date +"%H:%M:%S")$c9"
  PS1="$PS1 \$ "
}
